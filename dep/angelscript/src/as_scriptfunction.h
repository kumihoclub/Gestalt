...
struct asSFunctionTraits
{
	...
	void SetTrait(asEFuncTrait trait, bool set_lock) { if (set_lock) traits |= trait; else traits &= ~trait; }
...
public:
	...
	void SetShared(bool set_lock) {traits.SetTrait(asTRAIT_SHARED, set_lock);}
	void SetReadOnly(bool set_lock) { traits.SetTrait(asTRAIT_CONST, set_lock); }
	void SetFinal(bool set_lock) { traits.SetTrait(asTRAIT_FINAL, set_lock); }
	void SetOverride(bool set_lock) { traits.SetTrait(asTRAIT_OVERRIDE, set_lock); }
	void SetProtected(bool set_lock) { traits.SetTrait(asTRAIT_PROTECTED, set_lock); }
	void SetPrivate(bool set_lock) { traits.SetTrait(asTRAIT_PRIVATE, set_lock); }
