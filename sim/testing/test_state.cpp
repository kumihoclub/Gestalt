...
void TestState::onEnter() {
	...
	test_text.set("\"Game World\"");
	test_text.transform.scale = { 0.25f, 0.25f };
	test_text.transform.anchor = SP_ANCHOR_VIEWPORT;
	test_text.transform.pos = { -1.0f, 1.0f };
	test_text.transform.origin = { -1.0f, 1.0f };
...
void TestState::update(SPViewport& view, SPFrame& frame, BTTransitionStack* transition_stack) {
	...
	if (arrow_up.pressed()) {
		view_scale *= 2;
		view.size.x = (480.0f * view_scale);
		view.size.y = (270.0f * view_scale);
	}
	...
	if (vel == glm::vec2(0.0f, 0.0f)) {
		...
		step_alarm.pause();
		target_frame = 0;
		mako_prop.material.get().sprite = target_frames[target_frame];
	}
