...
int asCGarbageCollector::AddScriptObjectToGC(void *obj, asCObjectType *objType)
{
	...
	// Invoke the garbage collector to cleanup a little garbage as new comes in
		...
		case destroyGarbage_haveMore:
		{
			...
			else
			{
				...
				else
				{
					...
					// is no more garbage to cleanup at the moment
					return 0;
				}
			}
		}
		...
		case destroyGarbage_haveMore:
		{
			...
			if( ++destroyOldIdx < gcOldObjects.GetLength() )
			{
				...
				if( gcObj.type->beh.gcGetRefCount == 0 )
				{
					...
					// Just remove the object, as we will not bother to cleanup it
					numDestroyed++;
