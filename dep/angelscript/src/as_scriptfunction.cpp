...
asCScriptFunction::asCScriptFunction(asCScriptEngine *engine, asCModule *mod, asEFuncType _funcType)
{
	...
	if( funcType == asFUNC_DELEGATE )
	{
		...
		externalRefCount.set_lock(1);
		internalRefCount.set_lock(0);
	}
	else
	{
		internalRefCount.set_lock(1);
		externalRefCount.set_lock(0);
	}
			...
			case asBC_ALLOC:
				{
					...
					if( funcId > 0 )
					{
						...
						// The engine may have been forced to cleanup the function internals early
